! 2-D FitzHugh-Nagumo with a learned time-stepper on 3x3 patches, Fortran.
!
! The twin of react.c: gather every cell's patch, ONE batched call over the
! whole field, scatter back; weights loaded by stepper_init once, before
! the loop; feat and out mapped once and handed over as device addresses.
!
! The batched call goes to the C archive (libstepper.a, built for the
! toolchain's native backend) through the module's bind(C) interfaces:
! stepper_infer_batch_dev takes c_ptrs to device memory, which
! `target data use_device_addr` makes c_loc() produce, and stepper_sync_dev
! is the wait a host with no stream of its own needs before its next target
! region reads the result (a no-op for an omp-backend archive).
program react
...
#define NX 256
#endif
    integer, parameter :: nx_ = NX, ncell = nx_ * nx_        ! grid; -DNX=64 for a quick host run
    ...
    ! Plan step, once, before anything is mapped or timed. This is the C
    ! archive's init (stepper_init_dev binds to it): the batched call below
    ! runs the archive's kernel, and only the archive's init uploads the
    ! archive's copy of the weights. The module's own stepper_init would
    ! fill the module's arrays for the Fortran per-point path, unused here.
    status = stepper_init_dev('stepper.rwt' // c_null_char)
    if (status /= 0) then
    ...
    print '(A,F8.1,A,F6.2,A)', '  surrogate        ', 1e3 * t_nn, ' ms  (', 1e3 * t_nn / nbig, &
        ' ms per big step: gather, infer_batch, scatter)'
    print '(A,ES10.3)', '  relative L2 error of the surrogate vs the reference: ', err
    if (.not. (err < tol)) then
...
contains
    ...
    ! Explicit-shape dummies throughout (see A's README on descriptors).
    subroutine gather(u, v, feat)
        ...
        do j = 1, nx_
            do i = 1, nx_
                ! Same order as train.py's patches(): u's 3x3 row-major (first index
                ! slowest, as react.c's i), then v's. Cell c is react.c's i*NX + j.
                c = (i - 1) * nx_ + j
                do di = -1, 1
                    do dj = -1, 1
    ...
    subroutine surrogate(u, v, feat, out, status)
        ...
        do s = 1, nbig
            ...
            ! feat and out are already on the device: use_device_addr makes
            ! c_loc() yield their device addresses for the C archive's kernel.
            !$omp target data use_device_addr(feat, out)
            status = stepper_infer_batch_dev(ncell, c_loc(feat), c_loc(out), c_null_ptr)
            !$omp end target data
            if (status /= 0) return
            status = stepper_sync_dev(c_null_ptr)    ! order the launch before the scatter
            if (status /= 0) return
            call scatter(out, u, v)
        end do
    end subroutine

    ! The same deterministic held-out initial condition as react.c.
