...
filePath = "../goldenFiles/maxpool_nonsquare/"
with open(filePath+"maxpool_nonsquare.txt", "w") as f2:
...
torch.onnx.export(model,               # model being run
                  ...
                  filePath+"maxpool_nonsquare.onnx",   # where to save the model (can be a file or file-like object)
...
torch.onnx.export(model,               # model being run
                  inp,                         # model input (or a tuple for multiple inputs)
                  filePath+"maxpool_nonsquare_weights.onnx",   # where to save the model (can be a file or file-like object)
                  export_params=True, dynamo=False,        # store the trained parameter weights inside the model file
                  opset_version=10,          # the ONNX version to export the model to
