...
class NN(nn.Module):
    def __init__(self):
        ...
        self.maxpool = nn.MaxPool2d(3, padding=[1,1])
...
filePath = "goldenFiles/maxpool_padding/"
with open(filePath+"maxpool_padding.txt", "w") as f2:
...
torch.onnx.export(model,               # model being run
                  ...
                  filePath+"maxpool_padding.onnx",   # where to save the model (can be a file or file-like object)
...
torch.onnx.export(model,               # model being run
                  inp,                         # model input (or a tuple for multiple inputs)
                  filePath+"maxpool_padding_weights.onnx",   # where to save the model (can be a file or file-like object)
                  export_params=True,        # store the trained parameter weights inside the model file
                  opset_version=10,          # the ONNX version to export the model to
