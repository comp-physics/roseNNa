...
GEMM_SMALL = "../goldenFiles/gemm_small/gemm_small.onnx"

def test_loads_nodes_in_order():
    g = load_graph(GEMM_SMALL)
    assert [n.op for n in g.nodes] == ["Gemm", "Relu", "Gemm", "Relu"]
    assert g.name == "gemm_small"

def test_every_shape_is_literal():
    g = load_graph(GEMM_SMALL)
