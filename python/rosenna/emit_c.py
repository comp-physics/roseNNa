...
def _emit_header(plan: Plan, ctype: str) -> str:
    ...
    lines += [
        ...
        f"   Returns 0, or 10 if a device allocation or copy failed in {m}_init",
        "   (cuda/hip backend of a file-loaded model; init never having been",
        "   called counts as that). */",
        f"int {m}_infer_batch(int n, const {ctype} *x, {ctype} *y, void *stream);",
        "",
        "#if defined(__cplusplus)",
        "}",
        "#endif",
...
def _emit_device_weight_table(plan: Plan, ctype: str) -> list:
    ...
    lines = [
        ...
        f"   __constant__ table of the device pointers, filled once by {m}_init",
        f"   through {_device_bind(m)}. Device code reads its own translation",
...
def _emit_source_head(plan: Plan, ctype: str) -> list:
    ...
    lines = [
        "/* Generated by rosenna. Do not edit. */",
        "",
        "/* On a cuda/hip build the runtime is reached only through the macros in",
        "   rosenna_rt.h, included ahead of the model header so that the CUDA/HIP",
        "   declaration keywords the header uses are in scope; a host C compiler",
        "   never sees it. */",
        _CUDA_GUARD,
...
def _emit_upload(plan: Plan) -> list:
    ...
    rosenna_rt.h. A repeated init frees the previous copies first (freeing a
